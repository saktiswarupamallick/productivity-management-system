...
class PgInsertBase extends import_query_promise.QueryPromise {
  ...
  onConflictDoNothing(config = {}) {
    if (config.target === void 0) {
      ...
      this.config.onConflict = import_sql.sql`(${import_sql.sql.raw(targetColumn)}) do nothing${whereSql}`;
    }
    return this;
  }
  /**
  ...
  onConflictDoUpdate(config) {
    ...
    let targetColumn = "";
    targetColumn = Array.isArray(config.target) ? config.target.map((it) => this.dialect.escapeName(it.name)).join(",") : this.dialect.escapeName(config.target.name);
    this.config.onConflict = import_sql.sql`(${import_sql.sql.raw(targetColumn)}) do update set ${setSql}${whereSql}`;
    return this;
  }
  /** @internal */
  getSQL() {
